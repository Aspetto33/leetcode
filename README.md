...
​	n = 1 和 0是两个特殊情况，0的时候是false，1的时候是true，循环n不等于1，如果n除2的余数不等于1，break，令n等于n/2。如果n等于1 返回true，否则返回false。
...
​	使用两个栈，一个作为辅助栈，当pop的时候，将剩余的数据push进辅助栈的同时求剩余元素的最小值，完成之后将辅助栈的数据push进主栈中。
...
如果要在队尾添加数据则将数据push进主栈，如果要删除队头数据，则将主栈数据push进辅助栈，移除辅助栈的头元素。
...
​	双指针法，一个指针指向头结点的前边空节点处，一个指针指向头节点。反转指针指向，返回开始指向空节点的指针。
